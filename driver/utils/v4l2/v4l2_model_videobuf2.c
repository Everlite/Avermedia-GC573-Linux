...
// static void v4l2_model_setup_buffer_cb_context(struct vb2_buffer
// *vb,v4l2_model_buffer_info_t *buffer_cb_context);
...
static int
v4l2_model_qops_queue_setup(struct vb2_queue *q, unsigned int *num_buffers,
...
static int
v4l2_model_qops_queue_setup(struct vb2_queue *q, const struct v4l2_format *fmt,
...
#endif
{
  ...
  //    pixfmt =
  //    framegrabber_g_out_pixelfmt(v4l2m_context->framegrabber_handle); if
  //    (width == 0 || height == 0)
  //    {
  //        return -EINVAL;
  //    }
  framebufsize =
      framegrabber_g_max_framebuffersize(v4l2m_context->framegrabber_handle);
  framegrabber_g_out_framesize(v4l2m_context->framegrabber_handle, &width,
                               &height); // test
  printk("%s framebufsize %u, width %d, height %d\n", __func__, framebufsize,
         width, height);
  // if (framebufsize == 0)
  {
    // int width, height;
    ...
    //			     &width, &height);
    bytesperline =
        framegrabber_g_out_bytesperline(v4l2m_context->framegrabber_handle);
  ...
  if (v4l2m_context->pic_bmp_handle) {
    ...
    load_no_signal_image(v4l2m_context->pic_bmp_handle, vb2_context->image_data,
                         width, height);
  ...
  *num_planes = 1;
  ...
  if (callback_item->callback) {
    callback_item->data.u.queue_setup_info.buffer_num = *num_buffers;
...
static int v4l2_model_qops_buf_prepare(struct vb2_buffer *vb) {
  ...
  for (i = 0; i < vb->num_planes; i++) {
    ...
    case V4L2_MODEL_BUF_TYPE_VMALLOC:
      ...
      v4l2_buf->buf_descs[i] =
          kzalloc(sizeof(v4l2_model_buf_desc_t) * buffer_info->buf_count[i],
                  GFP_KERNEL);
      if (v4l2_buf->buf_descs[i]) {
        v4l2_model_buf_desc_t *buf_descs = v4l2_buf->buf_descs[i];
        buf_descs[0].addr = (unsigned long)vb2_plane_vaddr(vb, i);
    ...
    case V4L2_MODEL_BUF_TYPE_DMA_SG: {
      ...
      if (v4l2_buf->buf_descs[i]) {
        ...
        if (!dma_map_sg(v4l2m_context->dev, sgt->sgl, sgt->nents,
                        DMA_FROM_DEVICE))
    ...
    case V4L2_MODEL_BUF_TYPE_DMA_CONT:
      ...
      if (v4l2_buf->buf_descs[i]) {
        v4l2_model_buf_desc_t *buf_descs = v4l2_buf->buf_descs[i];
        buf_descs[0].addr = vb2_dma_contig_plane_dma_addr(vb, i);
        buf_descs[0].size = vb2_plane_size(vb, i);
      }
      buffer_info->buf_info[i] = v4l2_buf->buf_descs[i];
    ...
    vb2_set_plane_payload(vb, i, width * height * (pixfmt->depth >> 3));
  ...
  if (callback_item->callback) {
    v4l2_model_callback_parameter_t *parm = &callback_item->data;
    parm->u.buffer_prepare_info.buffer_info = &v4l2_buf->buffer_info;
    callback_item->callback(parm);
  }
...
static void v4l2_model_qops_buf_cleanup(struct vb2_buffer *vb) {
  ...
  for (i = 0; i < vb->num_planes; i++) {
    ...
    case V4L2_MODEL_BUF_TYPE_DMA_SG: {
      ...
      dma_unmap_sg(v4l2m_context->dev, sgt->sgl, sgt->nents, DMA_FROM_DEVICE);
...
{
  v4l2_model_context_t *v4l2m_context = vb2b_to_v4l2context(vb);
  // v4l2_model_vb2_context_t *vb2_context=(v4l2_model_vb2_context_t
  // *)v4l2m_context->vb2_context;
  v4l2_model_callback_item_t *callback_item =
...
void v4l2_model_queue_cancel(void *context) {
  ...
  while (!list_empty(&vb2_context->buffer_list)) {
    ...
    if ((framegrabber_status & (FRAMEGRABBER_STATUS_V4L_START_STREAMING_BIT |
...
void v4l2_model_next_buffer(v4l2_model_handle_t context,
  ...
  next_buffer = list_first_entry(&vb2_context->buffer_list,
...
void v4l2_model_buffer_done(v4l2_model_handle_t context) {
  ...
  if (1) {
    ...
    u8 *ptr = NULL;
    input_status =
        framegrabber_g_input_status(v4l2m_context->framegrabber_handle);
    ...
    if (input_status != FRAMEGRABBER_INPUT_STATUS_OK || hdcp_flag) {
      v4l2_model_vb2_image_type_e type =
          (input_status == FRAMEGRABBER_INPUT_STATUS_NO_SIGNAL)
              ? V4L2_MODEL_VB2_IMAGE_NO_SIGNAL
      ...
      if (type != vb2_context->image_type) {
        if (type == V4L2_MODEL_VB2_IMAGE_NO_SIGNAL) {
          ...
          framegrabber_g_out_framesize(v4l2m_context->framegrabber_handle,
                                       &width, &height);
          load_no_signal_image(v4l2m_context->pic_bmp_handle,
                               vb2_context->image_data, width, height);
          printk("load no signal done\n");
        } else if (type == V4L2_MODEL_VB2_IMAGE_COPY_PROTECTION) {
          int width, height;
          framegrabber_g_out_framesize(v4l2m_context->framegrabber_handle,
                                       &width, &height);
          load_copy_protection_image(v4l2m_context->pic_bmp_handle,
                                     vb2_context->image_data, width, height);
...
void v4l2_model_feed_video_data(v4l2_model_handle_t context, void *buffer,
  ...
  if (v4l2m_cxt) {
    ...
    if (!pframebuf_info->vbuf_info) {
      v4l2_model_next_buffer(v4l2m_cxt, &pframebuf_info->vbuf_info);
      // printk("framebuf change to
      // %p\n",pframebuf_info->vbuf_info->plane_vaddr[0]);
    }

    framegrabber_g_input_framesize(v4l2m_cxt->framegrabber_handle, &width,
                                   &height);
    if ((width == 0) || (height == 0)) {
      ...
      if (pframebuf_info->vbuf_info) {
        ...
        const framegrabber_pixfmt_t *pixfmt =
            framegrabber_g_out_pixelfmt(v4l2m_cxt->framegrabber_handle);
        u8 *framebuf;
        u8 *linebuf = NULL;
        unsigned line;
        unsigned offset;
  ...
  {                                                                            \
    if (lb - framebuf + s <= framebuf_size) {                                  \
      ...
      printk("copy may out of framebuf size %u offset %lu size %lu !\n",       \
        ...
        framemode =
            framegrabber_g_input_framemode(v4l2m_cxt->framegrabber_handle);
        bytesperline =
            framegrabber_g_out_bytesperline(v4l2m_cxt->framegrabber_handle);
        // bytesperline = width*pix_size;
        framegrabber_g_out_framesize(v4l2m_cxt->framegrabber_handle, &width,
                                     &height);

        framebuf = (u8 *)pframebuf_info->vbuf_info->buf_info[0]->addr;
        framebuf_size = pframebuf_info->vbuf_info->buf_info[0]->size;
        ...
        if (framemode == FRAMEGRABBER_FRAMEMODE_PROGRESS)
          line_stride = bytesperline;
        else if (framemode == FRAMEGRABBER_FRAMEMODE_INTERLACED)
        ...
        if (framemode == FRAMEGRABBER_FRAMEMODE_PROGRESS) {
          ...
          if (line >= (height / 2)) {
            line -= (height / 2);
            linebuf = &framebuf[bytesperline + line_stride * line];
          } else {
            linebuf = &framebuf[line_stride * line];
          }

          // printk("Interlace  line %d offset
          // %d\n",line,bytesperline+line_stride*line);
        }
        // printk("%dx%dx%d rcv_size %d line %d offset %d line_stride %d
        // bytesperline
        // %d\n",width,height,pix_size,pframebuf_info->rcv_size,line,offset,line_stride,bytesperline);
        frame_remain_size =
            (width * height * pix_size) - pframebuf_info->rcv_size;
        if (offset) {
          copy_size = (width * pix_size) - offset;
          if (remain_size < copy_size)
            copy_size = remain_size;
          copy_to_framebuffer(&linebuf[offset], buf, copy_size);
          ...
          if (frame_remain_size > remain_size) {
            ...
            while (remain_size >= copy_size) {
              ...
              if (framemode == FRAMEGRABBER_FRAMEMODE_PROGRESS) {
                ...
                if (pframebuf_info->rcv_size ==
                    (width * pix_size * height / 2)) {
                  linebuf = &framebuf[bytesperline];
                } else {
                  linebuf += line_stride;
                }
              }
            ...
            if (remain_size) {
              ...
              copy_to_framebuffer(linebuf, buf, remain_size);
              pframebuf_info->rcv_size += remain_size;
              remain_size = 0;
            }
          } else {
            ...
            copy_size = (width * pix_size);
            remain_size -= frame_remain_size;
            while (frame_remain_size >= copy_size) {
              // printk("%d memcpy %ld(%d) size
              // %d\n",__LINE__,linebuf-framebuf,(linebuf-framebuf)/bytesperline,copy_size);
              copy_to_framebuffer(linebuf, buf, copy_size);
              buf += copy_size;
            ...
            if (frame_remain_size) {
              ...
              // %d\n",__LINE__,linebuf-framebuf,(linebuf-framebuf)/bytesperline,remain_size);
              copy_to_framebuffer(linebuf, buf, remain_size);
              pframebuf_info->rcv_size += frame_remain_size;
              frame_remain_size = 0;
            }
            ...
            if (remain_size) {
              v4l2_model_next_buffer(v4l2m_cxt, &pframebuf_info->vbuf_info);
              if (pframebuf_info->vbuf_info == NULL) {
                printk("No video buffer to receive\n");
                break;
              } else {
                pframebuf_info->rcv_size = 0;
                framebuf = (u8 *)pframebuf_info->vbuf_info->buf_info[0]->addr;
                ...
                linebuf = framebuf;
                frame_remain_size = (width * height * pix_size);
              }
            }
          }
        } while (remain_size);
