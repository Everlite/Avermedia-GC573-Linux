...
static unsigned int v4l2_model_fops_poll(struct file *,
                                         struct poll_table_struct *);
static ssize_t v4l2_model_fops_read(struct file *, char __user *, size_t,
                                    loff_t *);
static ssize_t v4l2_model_fops_write(struct file *, const char __user *, size_t,
                                     loff_t *);
static long v4l2_model_ioctl(struct file *file, unsigned int cmd,
                             unsigned long arg);
static void v4l2_model_notify(struct v4l2_subdev *sd, unsigned int notification,
                              void *arg);
static void *v4l2_model_alloc(void);
static void v4l2_model_release(void *context);

// void v4l2_board_register_subdev(struct sagitta_dev *sdev, struct v4l2_device
// *v4l2_dev);
...
/* Legacy control ioctls removed in kernel 6.x; use control framework instead */
...
#endif
      memcpy(&context->device_info, device_info,
             sizeof(v4l2_model_device_setup_t));
...
static void v4l2_model_release(v4l2_model_handle_t context) {
  ...
  for (i = 0; i < HOOK_IOCTL_TYPE_NUM; i++) {
    ...
    do {
      ...
      item = list_first_entry_or_null(hook_list_head,
                                      v4l2_model_ioctl_hook_item_t, list);
#else
      item = (!list_empty(hook_list_head)
                  ? list_first_entry(hook_list_head,
                                     v4l2_model_ioctl_hook_item_t, list)
                  : NULL);
#endif
...
static int v4l2_model_fops_release(struct file *fp) {
  ...
  // v4l2_model_context_t *v4l2m_context =  (v4l2_model_context_t *)
  // video_drvdata(fp);
...
static unsigned int v4l2_model_fops_poll(struct file *fp,
                                         struct poll_table_struct *poll) {
  return vb2_fop_poll(fp, poll);
}

static ssize_t v4l2_model_fops_write(struct file *file, const char __user *buf,
                                     size_t count, loff_t *ppos) {
  return vb2_fop_write(file, (char __user *)buf, count, ppos);
}

static ssize_t v4l2_model_fops_read(struct file *file, char __user *buf,
                                    size_t count, loff_t *ppos) {
...
static long v4l2_model_ioctl(struct file *file, unsigned int cmd,
  ...
  long try;
  v4l2_model_context_t *v4l2m_context =
      (v4l2_model_context_t *)video_drvdata(file);
  struct list_head *hook_list_head = NULL;
  ...
  if (!list_empty(hook_list_head)) {
    list_for_each(pos, hook_list_head) {
      ...
      try = video_usercopy(ioctl_hook_item->data, cmd, arg,
                           (v4l2_kioctl)ioctl_hook_item->ioctl_hook_func);
      if (try != -EINVAL) {
        return try;
      }
    }
...
int v4l2_model_hook_ioctl(v4l2_model_handle_t v4l2_context,
  ...
  if (v4l2m_context) {
    ...
    do {
      ...
      ioctl_hook_item = (v4l2_model_ioctl_hook_item_t *)kzalloc(
          sizeof(*ioctl_hook_item), GFP_KERNEL);
...
static void v4l2_model_notify(struct v4l2_subdev *sd, unsigned int notification,
                              void *arg) {
  v4l2_model_context_t *v4l2m_context =
      container_of(sd->v4l2_dev, v4l2_model_context_t, v4l2_dev);

  printk("%s sd name %s owner %s\n", __func__, sd->name, sd->owner->name);
  framegrabber_notify(v4l2m_context->framegrabber_handle, sd->name,
                      notification, arg);
  ...
  //__func__,detected_timings.bt.width,
  //detected_timings.bt.height,detected_timings.bt.interlaced ==
  //V4L2_DV_INTERLACED ? 'i' : 'p'); 		v4l2_subdev_call(sd,video, s_dv_timings,
  //&detected_timings);
  //
  //
  ...
  //		input_fmt->is_interlace = detected_timings.bt.interlaced ==
  //V4L2_DV_INTERLACED;
  //
  //
  //	}
